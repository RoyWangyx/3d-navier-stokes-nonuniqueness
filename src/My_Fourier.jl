...
This routine returns the lower-triangular matrix `A` such that

    P_{2i+δ}(x) = sum_{j=0}^i A[i+1,j+1] T_{2j+δ}(x),

where `δ = 0` if `flag == false` (even block) and `δ = 1` if `flag == true`
(odd block).

The coefficients are obtained from the explicit expansion of Legendre
polynomials in terms of Chebyshev polynomials of the first kind:

    P_n(x)
      = sum_{k=0}^{⌊n/2⌋}
          ((1/2)_{n-k} (1/2)_k) / ((n-k)! k!)
          * (2 - δ_{n,2k}) * T_{n-2k}(x),

where `δ_{n,2k}` is the Kronecker delta. Equivalently, the coefficient of
`T_0` is not multiplied by 2, while all other terms are.

For parity-separated bases, this becomes:

Even case (`flag == false`):
    P_{2i}(x) = sum_{j=0}^i A[i+1,j+1] T_{2j}(x),

    A[i+1,j+1] = 2 * b_{i+j+1} * b_{i-j+1},    j >= 1,
    A[i+1,1]   =     b_{i+1}^2,

Odd case (`flag == true`):
    P_{2i+1}(x) = sum_{j=0}^i A[i+1,j+1] T_{2j+1}(x),

    A[i+1,j+1] = 2 * b_{i+j+2} * b_{i-j+1},

where

    b_m = (2m-3)!! / (2m-2)!!.

This is exactly why the even case divides the first column by 2: the
constant mode `T_0` carries the factor `1` instead of `2`.
...
This routine returns the lower-triangular matrix `A` such that
    ...
    T_{2i+δ}(x) = sum_{j=0}^i A[i+1,j+1] P_{2j+δ}(x),

where `δ = 0` if `flag == false` (even block) and `δ = 1` if `flag == true`
(odd block).

The coefficients are derived by viewing both Legendre and Chebyshev
polynomials as Gegenbauer polynomials:

    P_n(x) = C_n^{1/2}(x),

and then applying the Gegenbauer connection formula

    C_n^λ(x)
      = sum_{k=0}^{⌊n/2⌋}
          ((μ+n-2k)/μ)
          * ( (λ)_{n-k} (λ-μ)_k / (k! (μ+1)_{n-k}) )
          * C_{n-2k}^μ(x).

Taking `μ = 1/2` expresses `C_n^λ` in the Legendre basis, and then using
the Chebyshev limit

    T_n(x) = lim_{λ→0} (n / (2λ)) C_n^λ(x),    n >= 1,

gives the expansion of `T_n` in terms of `P_{n-2k}`.

The resulting coefficients are

    T_n(x) = sum_{k=0}^{⌊n/2⌋} c_{n,k} P_{n-2k}(x),    n >= 1,

with

    c_{n,k}
      = (n/2) * (2n-4k+1)
        * (n-k-1)! * (-1/2)_k / (k! * (3/2)_{n-k}),

and

    T_0(x) = P_0(x).

For parity-separated bases:

Even case (`flag == false`):
    T_{2i}(x) = sum_{j=0}^i A[i+1,j+1] P_{2j}(x),

    A[i+1,j+1]
      = i * (4j+1) * (i+j-1)! * (-1/2)_{i-j}
        / ((i-j)! * (3/2)_{i+j}),
    with `T_0 = P_0` for `i = 0`.

Odd case (`flag == true`):
    T_{2i+1}(x) = sum_{j=0}^i A[i+1,j+1] P_{2j+1}(x),

    A[i+1,j+1]
      = ((2i+1)/2) * (4j+3) * (i+j)! * (-1/2)_{i-j}
        / ((i-j)! * (3/2)_{i+j+1}).
